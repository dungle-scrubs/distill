...
Two further limits of scope, which are not blind spots so much as what this
check is about. It sees text *becoming* durable, not durable text being moved:
**publish** renames a **staging directory** into a **generation** and this check
has nothing to say about it. And an entry in `ALLOWED_WRITERS` licenses a
function, not a line - a second writer added inside an already-licensed function
is not a new finding here, which is why the licensed functions are the two the
emitter is made of and two that write no text at all.
...
# reason, not a note of where a writer happens to be: an entry without one is
# the same defect as an unreasoned signature exemption.
ALLOWED_WRITERS: dict[tuple[str, str], str] = {
    ("emit.py", "TextEmitter.emit"): (
        ...
        "check exists to concentrate writes into rather than to forbid them."
    ),
    ("emit.py", "TextEmitter.emit_atomically"): (
    ...
    ("bundle_store.py", "ExclusiveLock.take"): (
    ...
    ("bundle_store.py", "ExclusiveLock.probe"): (
...
def test_every_durable_write_is_the_emitter_or_licensed() -> None:
    ...
    unlicensed = [
        write
        for write in _package_durable_writes()
        if (write.module, write.qualname) not in ALLOWED_WRITERS
    ]

    assert not unlicensed, (
        "durable writers outside TextEmitter: "
        + "; ".join(str(write) for write in unlicensed)
        + ". Emit through distill.emit.EMITTER, or add the site to "
        "ALLOWED_WRITERS with the reason it is legitimate."
    )


def test_the_licence_list_names_only_writers_that_exist() -> None:
    """A licence for a writer that is gone is a licence nobody reviewed.
    ...
    licensing whatever is later written in a function of that name.
    """
    sites = {(write.module, write.qualname) for write in _package_durable_writes()}
    stale = sorted(key for key in ALLOWED_WRITERS if key not in sites)

    assert not stale, f"ALLOWED_WRITERS licenses sites that no longer write: {stale}"


def test_every_licence_records_a_reason() -> None:
    """A licence without a stated reason is not a recorded decision."""
    unexplained = sorted(key for key, reason in ALLOWED_WRITERS.items() if not reason.strip())
